...
void ecc_center(int kin, TString target, double insig, double outsig, TString outfolder){
  if(kin>5 && target=="H1"){
    exit(0);
  }
  //Load Runs
  ...
  c3->cd(0);
  em->Draw(Form("%s>>ecc_kin%d",emx.Data(),kin),emcut + TCut(Form((kin==16) ? "rpr.z>%f" : "rpl.z>%f" ,up_cut[it])) + TCut(Form((kin==16) ? "rpr.z<%f" : "rpl.z<%f" ,0)));
  cout << emcut + TCut(Form((kin==16) ? "rpr.z>%f" : "rpl.z>%f" ,up_cut[it])) + TCut(Form((kin==16) ? "rpr.z<%f" : "rpl.z<%f" ,0)) << endl;
  TCanvas *c4 = new TCanvas();
  c4->cd(0);
  em->Draw(Form("%s>>em_down",emx.Data()),emcut + TCut(Form((kin==16) ? "rpr.z>%f" : "rpl.z>%f" ,0)) + TCut(Form((kin==16) ? "rpr.z<%f" : "rpl.z<%f" ,down_cut[it])));
  cout << emcut + TCut(Form((kin==16) ? "rpr.z>%f" : "rpl.z>%f" ,0)) + TCut(Form((kin==16) ? "rpr.z<%f" : "rpl.z<%f" ,down_cut[it])) << endl;
