...
void ecc_separate(int kin, TString target, double insig, double outsig, TString outfolder, int iter=0){
  ...
  TH1D *ecc_plot = new TH1D(Form("ecc_kin%d_%s",kin,set.Data()),Form("Kinematic %d Endcap Contamination",kin),33,0,0.99);
  ...
  c3->cd(0);
  em->Draw(Form("%s>>ecc_kin%d_%s",emx.Data(),kin,set.Data()),emcut + TCut(Form((kin==16) ? "rpr.z>%f" : "rpl.z>%f" ,up_cut[it])) + TCut(Form((kin==16) ? "rpr.z<%f" : "rpl.z<%f" ,center)));
