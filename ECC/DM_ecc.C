...
void DM_ecc(int kin, TString target){
  //Load Runs
  ...
  emz->Fit("emdownfit","L","SAME",.12,.14);
  ...
  TFile *f = new TFile("DM_"+target+".root","UPDATE");
  ecc_plot->Write();
  f->Close();
}
