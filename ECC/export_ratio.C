void export_ratio(TString target1, TString target2, TString folder){
    ...
    for(int i=0; i<=y; i++){
      ...
      kin = (kin==17) ? 16 : kin;
      ofstream out(Form("%s/ratios/%s_%s_kin%d.dat",folder.Data(),target1.Data(),target2.Data(),kin));
      ...
      for(int k=0; k<33; k++){
        ...
        if(e1!=0 && e2!=0){
          ...
          out << ratio[i]->GetBinCenter(k) << "," << ((1.-e1)/(1.-e2)) << "," << error << endl;
        }
      }
      out.close();
    }
