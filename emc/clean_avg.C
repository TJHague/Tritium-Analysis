...
void clean_avg(int kin, TString folder, TString inf="fullKin", TString outf="cleanedKin"){
  if(kin<=5){
    ...
    TH1D *fxemc = (TH1D*) gDirectory->Get("emcxavg");
    TH1D *femc = (TH1D*) gDirectory->Get("emc");
    fxemc->SetDirectory(0);
    femc->SetDirectory(0);
    delete f;

    bool first = true;
    TH1D *cleaned = new TH1D("emcxavg","Average binned x value - EMC Ratio",fxemc->GetNbinsX(),fxemc->GetXaxis()->GetXmin(),fxemc->GetXaxis()->GetXmax());
    cleaned->Sumw2();
    for(int i = 1; i <= fxemc->GetNbinsX(); i++){
      double c = fxemc->GetBinContent(i);
      if(c != 0){
        if(!first){
          double d = fxemc->GetBinContent(i+1);
          if(d != 0){
    ...
    TH1D *i1 = (TH1D*) gDirectory->Get("emcxavg");
    TH1D *emc1 = (TH1D*) gDirectory->Get("emc");
    i1->SetDirectory(0);
    emc1->SetDirectory(0);
    delete f;

    TFile *g = new TFile(Form("%s/%s/kin%d_2nd.root",folder.Data(),inf.Data(),kin));
    TH1D *i2 = (TH1D*) gDirectory->Get("emcxavg");
    TH1D *emc2 = (TH1D*) gDirectory->Get("emc");
    i2->SetDirectory(0);
    emc2->SetDirectory(0);
    delete g;
    ...
    TH1D *emc = new TH1D("combined","Average binned x value - EMC Ratio",i1->GetNbinsX(),i1->GetXaxis()->GetXmin(),i1->GetXaxis()->GetXmax());
    emc->Sumw2();
    ...
    for(int i = 1; i <= i1->GetNbinsX(); i++){
      if(i1->GetBinContent(i)!=0 || i2->GetBinContent(i)!=0){
        ...
        double e1 = emc1->GetBinError(i);
        double e2 = emc2->GetBinError(i);
    ...
    bool first = true;
    TH1D *cleaned = new TH1D("emcxavg","Average binned x value - EMC Ratio",emc->GetNbinsX(),emc->GetXaxis()->GetXmin(),emc->GetXaxis()->GetXmax());
    cleaned->Sumw2();
    ...
    TH1D *i3 = (TH1D*) gDirectory->Get("emcxavg");
    TH1D *emc3 = (TH1D*) gDirectory->Get("emc");
    i3->SetDirectory(0);
    emc3->SetDirectory(0);
    delete h;
