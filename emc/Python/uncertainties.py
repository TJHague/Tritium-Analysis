...
def H2H1ECCerror(x):
  ...
  err = x*((x*0.41158059) - 0.1013631)
  err += ((x*(-0.1013631)) + 0.02588159)
  err *= (ex*ex)
...
def H3He3ECCerror(x):
  ...
  err = x*((x*0.4762095) - 0.11694056)
  err += ((x*(-0.11694056)) + 0.03015558)
  err *= (ex*ex)

  return err**(1/2)

def H3H2ECCerror(x):
    return np.sqrt(H3He3ECCerror(x)**2 + He3H2ECCerror(x)**2)
