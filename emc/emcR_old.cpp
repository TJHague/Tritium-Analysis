...
void emcR(Int_t kin, TString folder, Int_t nbins, Double_t low, Double_t high, Int_t iter=0){
  ...
  ****************************************************************************/
  ...
  for(Int_t i=0; i<He3vec.size(); i++){
    ...
    Double_t Q, I, updated, T2, T2s, avgI=0, cer, prl1, prl2, ph, th, dp, z, x_bj, Q2, n, p, W2, fpx, fpth;
    //Double_t p[1] = {0};
    Int_t Iev=0;
    cout <<"ok1" << endl;
    ...
    for(Int_t j=0; j<events; j++){
      ...
      if(T2==1){
        trig_rec++;
        if((PID(cer, prl1, prl2, p, n, arm)==true)&&(ACC(ph, th, dp, fpx, fpth, arm)==true)&&(EC(z, arm)==true)&&(W2cut(W2)==true)){
