...
void merge_avg(TString folder, TString outf="He3", bool zero_err=false, TString inf="cleanedKin"){
  ...
  hist[0] = (TH1D*) gDirectory->Get("emcxavg");
  emcs[0] = (TH1D*) gDirectory->Get("emc");
  hist[0]->SetDirectory(0);
  emcs[0]->SetDirectory(0);
  delete a;

  TFile *b = new TFile(Form("%s/%s/kin1.root",folder.Data(),inf.Data()));
  hist[1] = (TH1D*) gDirectory->Get("emcxavg");
  emcs[1] = (TH1D*) gDirectory->Get("emc");
  hist[1]->SetDirectory(0);
  emcs[1]->SetDirectory(0);
  delete b;

  TFile *c = new TFile(Form("%s/%s/kin2.root",folder.Data(),inf.Data()));
  hist[2] = (TH1D*) gDirectory->Get("emcxavg");
  emcs[2] = (TH1D*) gDirectory->Get("emc");
  hist[2]->SetDirectory(0);
  emcs[2]->SetDirectory(0);
  delete c;

  TFile *d = new TFile(Form("%s/%s/kin3.root",folder.Data(),inf.Data()));
  hist[3] = (TH1D*) gDirectory->Get("emcxavg");
  emcs[3] = (TH1D*) gDirectory->Get("emc");
  hist[3]->SetDirectory(0);
  emcs[3]->SetDirectory(0);
  delete d;

  TFile *e = new TFile(Form("%s/%s/kin4.root",folder.Data(),inf.Data()));
  hist[4] = (TH1D*) gDirectory->Get("emcxavg");
  emcs[4] = (TH1D*) gDirectory->Get("emc");
  hist[4]->SetDirectory(0);
  emcs[4]->SetDirectory(0);
  delete e;

  TFile *f = new TFile(Form("%s/%s/kin5.root",folder.Data(),inf.Data()));
  hist[5] = (TH1D*) gDirectory->Get("emcxavg");
  emcs[5] = (TH1D*) gDirectory->Get("emc");
  hist[5]->SetDirectory(0);
  emcs[5]->SetDirectory(0);
  delete f;

  TFile *g = new TFile(Form("%s/%s/kin7.root",folder.Data(),inf.Data()));
  hist[6] = (TH1D*) gDirectory->Get("emcxavg");
  emcs[6] = (TH1D*) gDirectory->Get("emc");
  hist[6]->SetDirectory(0);
  emcs[6]->SetDirectory(0);
  delete g;

  TFile *h = new TFile(Form("%s/%s/kin9.root",folder.Data(),inf.Data()));
  hist[7] = (TH1D*) gDirectory->Get("emcxavg");
  emcs[7] = (TH1D*) gDirectory->Get("emc");
  hist[7]->SetDirectory(0);
  emcs[7]->SetDirectory(0);
  delete h;

  TFile *k = new TFile(Form("%s/%s/kin11.root",folder.Data(),inf.Data()));
  hist[8] = (TH1D*) gDirectory->Get("emcxavg");
  emcs[8] = (TH1D*) gDirectory->Get("emc");
  hist[8]->SetDirectory(0);
  emcs[8]->SetDirectory(0);
  delete k;

  TFile *l = new TFile(Form("%s/%s/kin13.root",folder.Data(),inf.Data()));
  hist[9] = (TH1D*) gDirectory->Get("emcxavg");
  emcs[9] = (TH1D*) gDirectory->Get("emc");
  hist[9]->SetDirectory(0);
  emcs[9]->SetDirectory(0);
  delete l;

  TFile *m = new TFile(Form("%s/%s/kin15.root",folder.Data(),inf.Data()));
  hist[10] = (TH1D*) gDirectory->Get("emcxavg");
  emcs[10] = (TH1D*) gDirectory->Get("emc");
  hist[10]->SetDirectory(0);
  emcs[10]->SetDirectory(0);
  delete m;

  TFile *n = new TFile(Form("%s/%s/kin16.root",folder.Data(),inf.Data()));
  hist[11] = (TH1D*) gDirectory->Get("emcxavg");
  emcs[11] = (TH1D*) gDirectory->Get("emc");
  hist[11]->SetDirectory(0);
  emcs[11]->SetDirectory(0);
  delete n;
  ...
  TH1D *emc = new TH1D("emcxavg","Average Binned x - EMC Ratio",hist[0]->GetNbinsX(),hist[0]->GetXaxis()->GetXmin(),hist[0]->GetXaxis()->GetXmax());
  ...
  for(int i = 1; i <= hist[0]->GetNbinsX(); i++){
    ...
    for(int j = 0; j <= 11; j++){
      ...
      if(c1 != 0){
        double e1 = emcs[j]->GetBinError(i);
        c += c1/(e1*e1);
        e += 1./(e1*e1);
      }
    }
